	...
	asmlinkage long interceptor(struct pt_regs reg) {
		...
		int monitored_pid = check_pid_monitored(reg.ax, current_pid);
 		if (table[reg.ax].intercepted == 1){
			printk(KERN_DEBUG "");

			if (table[reg.ax].monitored==0){  // syscall is not being monitored
				//log message with macro if pid is not monitored.
				log_message(current_pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
			}
			else if (table[reg.ax].monitored == 1){		// syscall is being monitored


				if (monitored_pid == 0){		// current pid isn't monitored so don't log message

					return table[reg.ax].f(reg);

				}else {
					if(monitored_pid==1){		// current pid is monitored so log message
						// log message with log macro
						log_message(current_pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
					}
				}
			}
	...
	asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	 	...
	 	if (cmd == REQUEST_SYSCALL_INTERCEPT){
			//The system must be non negative, not greater than NR_syscalls and not equal to MY_CUSTOM_SYSCALL
			if (syscall < 0 || syscall > NR_syscalls || (syscall == MY_CUSTOM_SYSCALL)){
				return -EINVAL;
			}
			//The cmd must be a root must be the root
			else if (current_uid() !=0){
				return -EPERM;
			}
			//if the system call is already intercepted
			...
			//if the system call is not intercepted yet
			...
			// To make sure pid is not a negative number
		    else if(pid < 0){
		 	   return -EINVAL;
		   } //if the pid is not 0, then the pid must exist
		    else if((pid != 0) && (pid_task(find_vpid(pid), PIDTYPE_PID) == 0)){
		 	   return -EINVAL;
		    }
			// it must be a root
		   else if(current_uid() != 0){
			   // To check is pid is 0 and it is not a root
			   if(pid == 0){
				   return -EPERM;
			   }
			   // to check if 2 pids have the same owner
			   else if(check_pid_from_list(current->pid, pid) != 0){
				   return -EPERM;
			   }
		   }

		   //  if the pid is being monitored for the syscall
		   if(check_pid_monitored(syscall, pid) == 1){
			   return -EBUSY;
		   } else {

			   spin_lock(&pidlist_lock);
			   //if pid cannot be added to the monitored list cause of memory issues,
			   // return -ENOMEM
		   ...
		   return 0;
			...
			//Check is the syscall is not intercepted
			if(table[syscall].intercepted == 0){
				return -EINVAL;
			}
			else{
				spin_lock(&pidlist_lock);
				//if the pid is not monitored, cannot stop monitoring it
	...
	static int init_function(void)
	{
		...
		// Lock table and allow read/write access
		spin_lock(&calltable_lock);
		set_addr_rw((unsigned long) sys_call_table);

		// Hijack MY_CUSTOM_SYSCALL and save the original
		orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];
		sys_call_table[MY_CUSTOM_SYSCALL] = &my_syscall;

		// Hijack exit_group and save the original
		orig_exit_group = sys_call_table[__NR_exit_group];
		sys_call_table[__NR_exit_group] = &my_exit_group;

		// Unlock table and allow readonly access
		set_addr_ro((unsigned long) sys_call_table);
		spin_unlock(&calltable_lock);

		// Initialize list and set initial tracking values for syscalls
		while (syscall_number < NR_syscalls){
			INIT_LIST_HEAD(&(table[syscall_number].my_list));
			table[syscall_number].intercepted = 0;
			table[syscall_number].monitored = 0;
			table[syscall_number].listcount = 0;
			syscall_number++;
		}
		return 0;
	}
	...
	static void exit_function(void)
	{
		...
		// detroy list keeping track of pids for all syscalls
		while (syscall_number < NR_syscalls){
			destroy_list(syscall_number);
			syscall_number++;
		}

		// lock and set table to writtable
		spin_lock(&calltable_lock);
		set_addr_rw((unsigned long)sys_call_table);

		// set default values in the table
		sys_call_table[MY_CUSTOM_SYSCALL] = orig_custom_syscall;
		sys_call_table[__NR_exit_group] = orig_exit_group;

		// unlock and set table to read-only
		set_addr_ro((unsigned long)sys_call_table);
		spin_unlock(&calltable_lock);
